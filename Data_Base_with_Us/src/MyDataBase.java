...
public class MyDataBase {
    public static String url ="jdbc:mysql://localhost/Practic_work";
    ...
    public void addVariant()
    {
        ...
        selectOneColum("id_student", "students", randomVar.idArray);
        randomVar.randVar();
        Connection connection = null;
        try {
            connection = openConnection();
            String query = "SELECT COUNT(*) FROM `studentsTest`";
            PreparedStatement statement = connection.prepareStatement(query);
            ResultSet resultSet = statement.executeQuery();
            if(resultSet.next())
            {
                ...
                {//Если есть записи в таблице, то удалить все
                    query = "DELETE FROM `studentsTest`";
                    statement.executeUpdate(query);
                }
                for (int i=0; i<randomVar.idArray.size();i++)
                {
                    query = "INSERT INTO `studentsTest` (`id_stud_test`, `id_student`, `var`) " +
